...
> a free-to-read finance reader in `ledger/` that loads public RSS feeds on
> the phone. Both are separate from the inventory system and not covered by
> this guide.
...
- **Every non-backup item with positive window need goes on the order**,
  ...
  depend on Reorder-at. The one exception is an explicit skip: an item
  whose order box holds a typed 0 stays off the order. (In Tue + Fri mode the old filter stands: at-reorder,
...
1. **Count** — three sort modes: **Walk** (grouped by station, matches the
   ...
   **Save count** stores a
   ...
   beats the suggestion — **including 0, which means "skip this item —
   order zero"**: the item drops off the order list, and the box keeps an
   amber "skipped · empty=auto" label so the skip is never silent.
   **Emptying the box puts the item back on automatic.** The override is
   sticky: re-counting the item, tapping ±, or changing Max does NOT
   clear it. **Mark as ordered clears all overrides** — they were
   consumed by the logged order, so the next cycle starts from the
   app's own math. The Order row's "your number, not the app's" line
